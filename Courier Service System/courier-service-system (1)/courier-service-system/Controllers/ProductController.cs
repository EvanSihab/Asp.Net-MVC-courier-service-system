...
namespace courier_service_system.Controllers
{
    public class ProductController : Controller
    {
        ...
        [HttpGet]
        public ActionResult EditProductType(string id)
        {
            if (Session["uname"] != null)
            {
                return View(this.productTypeService.Get(id));
            }
            return RedirectToAction("Index", "Home");
        }
        [HttpPost]
        public ActionResult EditProductType(ProductType productType)
        {
            if (Session["uname"] != null)
            {
                if (productType.Name.Length < 3)
                {
                    TempData["TypeMessage"] = "Type Name Should Contain Atleast 3 Characters";
                    return RedirectToAction("EditProductType", "Product", new { id = productType.Id });
                }
                else
                {
                    int i = productTypeService.UpdateProductType(productType);
                    if (i != 0)
                    { TempData["TypeMessage"] = "Successfully Updated"; }
                    return RedirectToAction("AllTypes", "Product");
                }
            }
            return RedirectToAction("Index", "Home");
        ...
        [HttpGet]
        public ActionResult DeleteProductType(string id)
        {
            if (Session["uname"] != null)
            {
                return View(this.productTypeService.Get(id));
        ...
        public ActionResult CreateCustomerReSceiver(Customer customer)
        {
            if (Session["uname"] != null)
            {
                if (Request["city"].ToString() == employeeService.Get(Session["id"].ToString()).OfficeId)
                {
                    TempData["DestinationError"] = "Receiving Office is Incorrect";
                    return RedirectToAction("TakeOder", "Product");
                }
                else
                {
                    customer.ReceiverAddress = Request["country"] + "," + Request["city"];
                    customer.CustomerId = customer.CustomerPhoneNumber + "-" + DateTime.Now.Day.ToString() +DateTime.Now.Month.ToString() + DateTime.Now.Year.ToString() + "-" + customerService.GetRowCount();
                    ...
                    if (tripCheck == null)
                    {
                        ...
                        if (tripCheck == null)
                        {
                            ...
                            if (tripCheck == null)
                            {
                                TempData["DestinationError"] = "Sorry, No Trips within 3 Days. Contact with Admin to Assign New trip";
                                return RedirectToAction("TakeOder", "Product");
                    ...
                    if (tripCheck == null)
                    {
                        ...
                        if (tripCheck == null)
                        {
                            ...
                            if (tripCheck == null)
                            {
                                ...
                                TempData["DestinationError"] = "Sorry, No Trips Available within 3 Days. Contact with Admin to Assign New trip";
                                return RedirectToAction("TakeOder", "Product");
                                //Session["ProductTripId"] = "No Available Trip";
                            }
                            else
